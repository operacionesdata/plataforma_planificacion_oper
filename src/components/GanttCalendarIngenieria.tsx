...
import { Save, Plus, X, ChevronDown, ChevronUp, Edit3, AlertCircle } from "lucide-react";
import {
  saveToSupabase,
  loadFromSupabase,
  saveSharedFines,
...
// Configuración de actividades - SOLO LAS 3 CON PORCENTAJE
const actividades = [
  { number: "1", code: "ESTOC", name: "Estudio Oceanográfico y Ambiental", color: "#22d3ee", hasPercentage: true },
  { number: "2", code: "ELMCF", name: "Elaboración de MCF + Planos Ingeniería", color: "#a3e635", hasPercentage: true },
  { number: "3", code: "CHAIN", name: "Charla Internalización Producción", color: "#fdba74", hasPercentage: true },
...
export const GanttCalendarIngenieria: React.FC = () => {
  const { canView, canEditProyectado, canEditRealizado, checkEditPermission, getAuthorName, hasAnyEditPermission, profile, isAdmin } = useGanttPermissions('ingenieria');
  
  const [numJaulas, setNumJaulas] = useState<number>(7);
  const [selectedActivity, setSelectedActivity] = useState<string>("ESTOC");
  ...
  // Estados para el sistema de comentarios con información del autor
  ...
  const saveWeekComment = async () => {
    if (editingWeek) {
      ...
      await saveToSupabase("animacion-ingenieria", {
        cellData,
        centroNames: {},
        barcoNames,
        selectedCentros,
  ...
  const handleCellClick = async (centroId: number, rowType: string, dateIndex: number) => {
    ...
    if (isRecursosRow) {
      ...
      if (selectedActivity === "🗑️") {
        ...
        if (esDesviacion) {
          // Las desviaciones siempre duran 1 día (no usan desplazamiento en realizado)
          const newData = { ...cellData, [key]: selectedActivity };
          setCellData(newData);
        } else {
          // Actividades normales con porcentaje (duran 1 día)
          ...
          if (rowType.includes("proyectado")) {
            // Verificar si hay una actividad en esta celda
            if (newData[key]) {
              // Si hay una actividad, desplazarla
  ...
  const saveData = async () => {
    ...
    const result = await saveToSupabase("animacion-ingenieria", dataToSave);

    setIsSyncing(false);
    setLastSyncStatus(result.success ? "success" : "error");

    const button = document.getElementById("save-button-ingenieria");
    if (button) {
  ...
  const loadData = async () => {
    ...
    const result = await loadFromSupabase("animacion-ingenieria");
  ...
  return (
    <div className="ocean-card space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Calendario Tipo Gantt - Ingeniería </h2>
