...
// Configuración de Actividades SSO
const actividades = [
  { number: "1", code: "INSFRA", name: "Instalación Franja", color: "#95b8f6", diasFijos: 8, hasPercentage: false },
  {
    number: "2",
    code: "INSCER",
    name: "Instalación Cerco Perimetral",
    color: "#add5fa",
    diasFijos: 9,
    hasPercentage: false,
  },
  { number: "3", code: "EXTINT", name: "Extintores", color: "#fa5f49", diasFijos: 1, hasPercentage: true },
  { number: "4", code: "CHALEC", name: "Chalecos Salvavidas", color: "#f9a59a", diasFijos: 1, hasPercentage: true },
  { number: "5", code: "AROSAL", name: "Aros Salvavidas", color: "#f9d99a", diasFijos: 1, hasPercentage: true },
  {
    number: "6",
    code: "CAMCUF",
    name: "Camilla, Cuello Ortopédico y Frazada",
    color: "#10b981",
    diasFijos: 1,
    hasPercentage: true,
  },
  { code: "🗑️", name: "Borrar", color: "#F1F5F9", diasFijos: 0, hasPercentage: false },
];
...
export const GanttCalendarSSO: React.FC = () => {
  const { canView, canEditProyectado, canEditRealizado, checkEditPermission, getAuthorName, hasAnyEditPermission, profile, isAdmin } = useGanttPermissions('sso');
  
  const [selectedActivity, setSelectedActivity] = useState<string>("INSFRA");
  const [selectedRecurso, setSelectedRecurso] = useState<string>("BZU");
  ...
  // Estados para porcentajes de actividades
  ...
  const saveWeekComment = async () => {
    if (editingWeek) {
      ...
      await saveToSupabase("animacion-sso", {
        cellData,
        centroNames: {},
      ...
      if (selectedActivity === "🗑️") {
        ...
        if (rowType.includes("realizado")) {
          ...
          if (actividad?.hasPercentage) {
            const percentage = activityPercentages[selectedActivity] || 100;
            newData[key] = `${selectedActivity}:${percentage}`;
          } else {
          ...
          if (esDesviacion) {
            ...
            // Actividades normales con días fijos
            const actividad = actividades.find((a) => a.code === selectedActivity);
            const diasTotales = actividad?.diasFijos || 1;
  ...
  const saveData = async () => {
    ...
    const result = await saveToSupabase("animacion-sso", dataToSave);

    setIsSyncing(false);
    setLastSyncStatus(result.success ? "success" : "error");

    const button = document.getElementById("save-button-sso");
    if (button) {
  ...
  const loadData = async () => {
    setIsSyncing(true);
    clearAllLocalStorage();
    const result = await loadFromSupabase("animacion-sso");
    ...
    if (result.success && result.data) {
      ...
      if (result.data.activityPercentages) {
        setActivityPercentages(result.data.activityPercentages);
      }
  ...
  return (
    <div className="ocean-card space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Calendario Tipo Gantt - Área SSO</h2>
      ...
      <div className="p-4 rounded-lg" style={{ background: "#2c3e50" }}>
        ...
        {showActivitySelector && (
          <div className="mt-4 space-y-4 animate-in slide-in-from-top duration-200">
            <h3 className="font-medium text-white">Actividades SSO:</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {actividades
                ...
                .map((actividad) => {
                  ...
                  const percentage = activityPercentages[actividad.code] || 100;
                    ...
                    <div key={actividad.code} className="space-y-2">
                      <button
                        ...
                        className={`w-full px-4 py-3 rounded-lg text-sm font-medium transition-all ${
                          isSelected ? "bg-orange-500 text-white" : "bg-slate-600 text-white hover:bg-slate-500"
                        }`}
                        title={`${actividad.name} - ${actividad.diasFijos} día${actividad.diasFijos > 1 ? "s" : ""}`}
                      >
                        {actividad.number} - {actividad.name}
                        <div className="text-xs mt-1">
                          ({actividad.diasFijos} día{actividad.diasFijos > 1 ? "s" : ""}
                          {actividad.hasPercentage ? ` - ${percentage}%` : ""})
                        </div>
                      </button>

                      {isExpanded && actividad.hasPercentage && (
                        <div className="bg-slate-700 p-3 rounded-lg space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              max="100"
                              value={percentage}
                              onChange={(e) => handlePercentageChange(actividad.code, parseInt(e.target.value) || 0)}
