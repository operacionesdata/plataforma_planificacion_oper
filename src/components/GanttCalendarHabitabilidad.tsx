...
import { Save, ChevronDown, ChevronUp, Plus, X, Edit3, AlertCircle } from "lucide-react";
import {
  saveToSupabase,
  loadFromSupabase,
  saveSharedFines,
...
// Colores unificados para la Gantt
const GANTT_COLORS = {
  COLOR_1: "#95b8f6",
  COLOR_2: "#add5fa",
  COLOR_3: "#fa5f49",
...
const actividades = [
  { number: "1", code: "GEN", name: "Generadores", color: GANTT_COLORS.COLOR_1, hasPercentage: true },
  {
    number: "2",
    code: "PTAG",
    name: "Planta de tratamiento aguas grises",
    color: GANTT_COLORS.COLOR_2,
    hasPercentage: true,
  },
  {
    number: "3",
    code: "PTAL",
    name: "Planta de tratamiento de aguas limpias",
    color: GANTT_COLORS.COLOR_3,
    hasPercentage: true,
  },
  { number: "4", code: "TDSL", name: "TK diésel", color: GANTT_COLORS.COLOR_4, hasPercentage: false },
  { number: "5", code: "EMBC", name: "Embarcaciones", color: GANTT_COLORS.COLOR_5, hasPercentage: false },
  { number: "6", code: "ENSO", name: "Ensilaje Operativo", color: GANTT_COLORS.COLOR_6, hasPercentage: false },
  {
    number: "7",
    code: "MOBP",
    name: "Mantencion Obra Gruesa Ponton / Base en tierra",
    color: GANTT_COLORS.COLOR_7,
    hasPercentage: false,
  },
  {
    number: "8",
    code: "MOBM",
    name: "Mantencion Obras Menores Ponton/ Base en tierra",
    color: GANTT_COLORS.COLOR_8,
    hasPercentage: false,
  },
  {
    number: "9",
    code: "SABA",
    name: "Sistema Alarmas y Bombas Achique (Red Incendio)",
    color: GANTT_COLORS.COLOR_9,
    hasPercentage: false,
  },
  { number: "10", code: "LNBL", name: "Línea blanca", color: GANTT_COLORS.COLOR_10, hasPercentage: false },
  { number: "11", code: "MOBL", name: "Mobiliario", color: GANTT_COLORS.COLOR_11, hasPercentage: false },
  { code: "🗑️", name: "Borrar", color: "#F1F5F9", hasPercentage: false },
];
...
const barcos = [
  { number: "1", code: "BZU", name: "Bza. Santa Úrsula", color: GANTT_COLORS.COLOR_1 },
  { number: "2", code: "LMC", name: "LM Caleb", color: GANTT_COLORS.COLOR_2 },
  { number: "3", code: "CAT", name: "Cat Estero", color: GANTT_COLORS.COLOR_3 },
  { number: "4", code: "CON", name: "Cono", color: GANTT_COLORS.COLOR_4 },
  { number: "5", code: "LML", name: "LM Levi", color: GANTT_COLORS.COLOR_5 },
  { number: "6", code: "ISC", name: "Isaí Cat", color: GANTT_COLORS.COLOR_6 },
  { number: "7", code: "IL", name: "Isla Lemu", color: GANTT_COLORS.COLOR_7 },
  { number: "8", code: "CU", name: "Cuadrilla", color: GANTT_COLORS.COLOR_8 },
...
export const GanttCalendarHabitabilidad: React.FC = () => {
  const { canView, canEditProyectado, canEditRealizado, checkEditPermission, getAuthorName, hasAnyEditPermission, profile, isAdmin } = useGanttPermissions('habitabilidad');
  
  const [selectedActivity, setSelectedActivity] = useState<string>("GEN");
  ...
  const [activityDurations, setActivityDurations] = useState<ActivityDurations>({
    GEN: 1,
    PTAG: 1,
    PTAL: 1,
    TDSL: 1,
  ...
  const [activityPercentages, setActivityPercentages] = useState<ActivityPercentages>({
    GEN: 0,
    PTAG: 0,
    PTAL: 0,
  });
  ...
  // Calcular métricas incluyendo TODAS las filas de proyectado y realizado
        ...
        group.rows.forEach((row) => {
          if (row.id.includes("proyectado")) {
            dates.forEach((date, dateIndex) => {
              const keyProyectado = `${centroId}-${row.id}-${dateIndex}`;
              if (cellData[keyProyectado]) {
                planificado++;
              }
            });
          }
          if (row.id.includes("realizado")) {
            dates.forEach((date, dateIndex) => {
              const keyRealizado = `${centroId}-${row.id}-${dateIndex}`;
              const valorRealizado = cellData[keyRealizado];

              if (valorRealizado) {
                // Contar TODOS los días en realizado (incluyendo desviaciones)
                totalRealizadoConDesviaciones++;

                // Extraer código de actividad
                const activityCode = valorRealizado.includes(":") ? valorRealizado.split(":")[0] : valorRealizado;

                // Contar solo trabajo real (sin desviaciones)
                if (!esActividadDesviacion(activityCode)) {
                  ejecutadoSinDesviaciones++;
                }

                // Contar desviaciones para el panel de análisis
                if (activityCode === "NAVEG") diasNavegacion++;
                else if (activityCode === "PTOCE") diasPuertoCerrado++;
                else if (activityCode === "RELEV") diasRelevo++;
                else if (activityCode === "INSSO") diasIncumplimientoSSO++;
              }
            });
          }
        });
  ...
  const saveWeekComment = async () => {
    if (editingWeek) {
      ...
      await saveToSupabase("animacion-habitabilidad", {
        cellData,
        centroNames: {},
        barcoNames,
        selectedCentros,
  ...
  // Manejar clic en celda - ACTUALIZADO CON DESPLAZAMIENTO
  const handleCellClick = async (centroId: number, rowType: string, dateIndex: number) => {
    ...
    if (isRecursosRow) {
      ...
      if (selectedActivity === "🗑️") {
        ...
        const diasTotales = activityDurations[selectedActivity] || 1;
        let newData = { ...cellData };

        // SI ES PROYECTADO: SIEMPRE desplazar actividades existentes
        ...
        for (let i = 0; i < diasTotales && dateIndex + i < dates.length; i++) {
          ...
          if (actividad?.hasPercentage) {
            const percentage = activityPercentages[selectedActivity] || 0;
            newData[newKey] = `${selectedActivity}:${percentage}`;
          } else {
  ...
  const getCellBackgroundColor = (centroId: number, rowType: string, dateIndex: number, date: Date) => {
    ...
    if (value) {
      ...
      if (isRecursosRow) {
        ...
        const activityCode = value.includes(":") ? value.split(":")[0] : value;
        const actividad = actividades.find((a) => a.code === activityCode);
        if (actividad) return actividad.color;

        const desviacion = actividadesDesviacion.find((d) => d.code === activityCode);
  ...
  const handleCentroNameChange = (centroId: number, newName: string) => {
    setCentroNames((prev) => ({
      ...prev,
      [centroId]: newName,
    }));
  ...
  const handleDurationChange = (activityCode: string, duration: number) => {
  ...
  const handlePercentageChange = (activityCode: string, percentage: number) => {
    setActivityPercentages((prev) => ({
      ...prev,
      [activityCode]: Math.min(100, Math.max(0, percentage)),
    }));
  };

  const toggleActivityExpanded = (activityCode: string) => {
  ...
  const saveData = async () => {
    ...
    const result = await saveToSupabase("animacion-habitabilidad", dataToSave);

    setIsSyncing(false);
    setLastSyncStatus(result.success ? "success" : "error");

    const button = document.getElementById("save-button-habitabilidad");
    if (button) {
  ...
  const loadData = async () => {
    setIsSyncing(true);
    clearAllLocalStorage();
    const result = await loadFromSupabase("animacion-habitabilidad");
    ...
    if (result.success && result.data) {
      ...
      setActivityDurations(result.data.activityDurations || activityDurations);
      setActivityPercentages(result.data.activityPercentages || activityPercentages);
  ...
  return (
    <div className="ocean-card space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Calendario Tipo Gantt - Habitabilidad </h2>
          ...
          <div className="mt-4 space-y-4 animate-in slide-in-from-top duration-200">
            ...
            <div className="grid grid-cols-3 gap-3">
              {actividades
                .filter((a) => a.code !== "🗑️")
                .map((actividad) => {
                  const isExpanded = expandedActivities[actividad.code];
                  const duration = activityDurations[actividad.code] || 1;
                  const percentage = activityPercentages[actividad.code] || 0;
                  const isSelected = selectedActivity === actividad.code;

                  return (
                    <div key={actividad.code} className="space-y-2">
                      <button
                        style={{
                          backgroundColor: isSelected ? actividad.color : "#475569",
                        }}
                        className="w-full px-4 py-3 rounded-lg text-sm font-medium transition-all text-white hover:opacity-80"
                        onClick={() => {
                          setSelectedActivity(actividad.code);
                          toggleActivityExpanded(actividad.code);
                        }}
                      >
                        <div className="text-center">
                          ...
                          {actividad.hasPercentage ? (
                            <div className="text-xs mt-1">({percentage}%)</div>
                          ) : (
                            <div className="text-xs mt-1">({duration}d)</div>
                          )}
                        </div>
                      </button>
                      ...
                      {isExpanded && (
                        <div className="bg-slate-700 p-3 rounded-lg space-y-2">
                          <div className="flex items-center gap-2">
                            <label className="text-white text-xs font-medium min-w-[60px]">Días:</label>
                            <input
                              type="number"
                              min="1"
                              max="30"
                              value={duration}
                              onChange={(e) => handleDurationChange(actividad.code, parseInt(e.target.value) || 1)}
                              className="bg-white border border-slate-300 rounded px-2 py-1 text-slate-900 w-16 text-sm"
                            />
                          </div>
                          ...
                          {actividad.hasPercentage && (
                            <div className="space-y-2">
                              <div className="flex items-center gap-2">
                                <label className="text-white text-xs font-medium min-w-[60px]">%:</label>
                                <input
                                  type="number"
                                  min="0"
                                  max="100"
                                  value={percentage}
                                  onChange={(e) =>
                                    handlePercentageChange(actividad.code, parseInt(e.target.value) || 0)
                                  }
                                  className="bg-white border border-slate-300 rounded px-2 py-1 text-slate-900 w-16 text-sm"
                                />
                              </div>
                              <input
                                ...
                                max="100"
                                value={percentage}
                                onChange={(e) => handlePercentageChange(actividad.code, parseInt(e.target.value))}
                                className="w-full"
                              />
                            </div>
                          )}
              ...
              <div key={centroId}>
                ...
                >
                  <input
                    ...
                    onChange={(e) => handleCentroNameChange(centroId, e.target.value)}
                          ...
                          {dates.map((date, dateIndex) => {
                            ...
                            if (value) {
                              ...
                              if (isRecursosRow) {
                                ...
                                const [activityCode, percentage] = value.includes(":")
                                  ? value.split(":")
                                  : [value, null];
                                const actividad = actividades.find((a) => a.code === activityCode);
                                const desviacion = actividadesDesviacion.find((d) => d.code === activityCode);
                                ...
                                tooltipText = actividad?.name || desviacion?.name || "";
                              }
                            }
