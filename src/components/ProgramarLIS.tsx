...
import { Lock, Plus, Trash2, Ship, Calendar, MapPin, Eye, EyeOff, Pencil, Save } from "lucide-react";
...
export interface LISProgramacion {
  id: string;
  centro: string;
  fechaInicio: string;
  fechaTermino: string;
...
export const ProgramarLIS = React.forwardRef<any, ProgramarLISProps>(({ onHasChanges, onSaveRequest }, ref) => {
  ...
  const [programaciones, setProgramaciones] = useState<LISProgramacion[]>([]);
  const [originalProgramaciones, setOriginalProgramaciones] = useState<LISProgramacion[]>([]);
  ...
  const [editingProg, setEditingProg] = useState<LISProgramacion | null>(null);
    ...
    guardarCambios: async () => {
      try {
        ...
        if (nuevas.length > 0) {
          ...
          const { error } = await supabase.from("lis_programaciones").insert(insertData);
          if (error) throw error;
        }

        if (eliminadas.length > 0) {
          for (const prog of eliminadas) {
            const { error } = await supabase.from("lis_programaciones").delete().eq("id", prog.id);
            if (error) throw error;
          }
        }
  ...
  const handleAgregarProgramacion = () => {
    ...
    const nuevasProgramaciones: LISProgramacion[] = selectedCentros.map((centro) => ({
  ...
  const openEditModal = (prog: LISProgramacion) => {
    setEditingProg(prog);
  ...
  const programacionesPorEmbarcacion = React.useMemo(() => {
    const agrupado = programaciones.reduce(
      ...
      {} as Record<string, LISProgramacion[]>,
    );
      ...
      <div className="w-full h-full flex items-center justify-center p-4">
        <Card className="bg-background/90 backdrop-blur-sm p-8 w-full max-w-md border border-border">
          <div className="flex flex-col items-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-2">Programar LIS</h2>
    ...
    <div className="w-full h-full flex flex-col gap-4 p-4">
      <Card className="bg-background/80 backdrop-blur-sm p-4 border border-border">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-foreground">Programar LIS</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="max-h-48 overflow-y-auto border border-border rounded-md p-2 bg-background/50">
              {filteredCentros.map((centro) => (
                <label
                  key={centro.name}
                  className="flex items-center gap-2 p-1 hover:bg-primary/10 rounded cursor-pointer"
                >
                  <input
                    type="checkbox"
          ...
          <div className="space-y-3">
            ...
            <Button onClick={handleAgregarProgramacion} className="w-full gap-2">
            ...
            {Object.entries(programacionesPorEmbarcacion).map(([embarcacion, progs]) => (
              <div key={embarcacion} className="border border-border rounded-lg p-4 bg-background/50">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <Badge variant="outline">{progs.length}</Badge>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <MapPin
                            className={`w-4 h-4 ${estado === "activo" ? "text-primary" : "text-muted-foreground"}`}
                          />
                          <div>
          ...
          {editingProg && (
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleEditarLocal} className="gap-2">
